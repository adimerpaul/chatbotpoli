...
async function send() {
  ...
  text.value=''
  const u = await apiFetch(`/api/conversations/${sel.value.id}/send`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:t})}).then(r=>r.json())
  store.upsert(u)
}
async function toggleResponder() {
  if (!sel.value) return
  const agente = isBotActive.value ? operatorName.value : 'Sin asignar'
  const u = await apiFetch(`/api/conversations/${sel.value.id}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({agente})}).then(r=>r.json())
  store.upsert(u)
}
